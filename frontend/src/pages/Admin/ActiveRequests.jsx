    ...
    const fetchRequests = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          setRequests(data.filter((request) => request.status == "Active"));
        } else {
            ...
            <tbody>
              {filteredRequests.map((request, index) => (
                <tr key={request.id || index} onClick={() => handleSelectRequest(request)}>
                  ...
                  <td>{request.email}</td>
                  <td>{request.streetAddress1}, {request.streetAddress2 ? (request.streetAddress2 + ", ") : ("")} {request.city}, {request.state} {request.zip}</td>
