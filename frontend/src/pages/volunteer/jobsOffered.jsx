...
const JobsOffered = () => {
  ...
  const acceptJob = async (jobId) => {
    try {
      const response = await axios.post("https://crsh-epic-disaster-resiliency-hub-client.vercel.app /volunteer/job/accept", { jobId });
  ...
  const rejectJob = async (jobId) => {
    try {
      const response = await axios.post("https://crsh-epic-disaster-resiliency-hub-client.vercel.app /volunteer/job/reject", { jobId });
      if (response.status === 200) {
  ...
  const completeJob = async (jobId) => {
    try {
      const response = await axios.post("https://crsh-epic-disaster-resiliency-hub-client.vercel.app /volunteer/job/job-completed", {
        jobId,
        details,
      });
      if (response.status === 200) {
