...
app.post("/create-account", async (req, res) => {
    try {
        ...
        let queryResult = await pool.query("SELECT * FROM AdminAccount WHERE email = $1", [req.body.email]);
...
app.post("/login", async (req, res) => {
    try {
        ...
        if (req.headers["authorization"]) {
            ...
            let queryResult = await pool.query("SELECT * FROM AdminAccount WHERE email = $1", [email]);
            if (queryResult.rows.length > 0) {
...
app.post("/create-volunteer/accept", async (req, res) => {
    ...
    try {
        let application = await pool.query("SELECT * FROM volunteerapplications WHERE id = $1", [req.body.id]);
        let result = pool.query(`INSERT INTO volunteer (email, password, assignment, first_name, last_name, phone_number, street_address, city, state, zip_code, admin_team, hospitality, logistic_tracking, community_outreach, community_helpers)
        ...
        let result2 = await pool.query("DELETE from volunteerapplications WHERE id = $1", [req.body.id]);
...
app.post("/create-volunteer/reject", async (req, res) => {
    try {
        let result = await pool.query("UPDATE volunteerapplications SET status = 'rejected' WHERE email = $1", [req.body.email]);
...
app.get("/create-volunteer/applications", async (req, res) => {
    try {
        let result = await pool.query("SELECT * FROM volunteerapplications WHERE status != 'rejected'");
...
app.post("/assign-volunteer/list", async (req, res) => {
    const { team } = req.body;
    let filteredVolunteers = await pool.query(`SELECT id, first_name, last_name, email, phone_number FROM "volunteer" WHERE "${team}" = TRUE`);
    res.status(200).json({
...
app.patch("/volunteers/volunteer-details", async (req, res) => {
    try {
        ...
        let result2 = await pool.query("SELECT * FROM volunteer WHERE id = $1", [parseInt(req.body.id)]);
...
app.post("/assign-volunteer/updateAssignment", async (req, res) => {
    ...
    try {
        await pool.query('UPDATE "volunteer" SET "offered" = $1 WHERE "id" = $2', [assignment, volId]);
...
app.get("/volunteers/volunteer-details/:id", async (req, res) => {
    try {
        let volunteer = await pool.query("SELECT * FROM volunteer WHERE id = $1", [parseInt(req.params.id)]);
...
app.get("/homeowner-requests", async (req, res) => {
    try {
        let requests = await pool.query("SELECT request_id, first_name, last_name, email, street_address_1, city, state, zip_code, status FROM request;");
