...
app.post("/login", async (req, res) => {
    try {
        ...
        if (req.headers["authorization"]) {
            ...
            let queryResult = await pool.query("SELECT * FROM Volunteer WHERE email = $1", [email]);
...
app.post("/status", (req, res) => {
    ...
    // Create a new instance of VolunteerApplication
    const index = dummyApplications.findIndex((applicant) => applicant.firstName.toLowerCase() == firstName.toLowerCase() &&
...
app.post("/job-accept", async (req, res) => {
    try {
        ...
        if (!offered || !action || !id) {
            return res
                .status(400)
                .send("Missing required parameters: 'offered', 'action', or 'id'.");
        }
        if (action === "reject") {
            // Reject the job for a specific volunteer
            await pool.query("UPDATE Volunteer SET offered = NULL WHERE offered = $1 AND id = $2", [offered, id]);
            res.status(200).send("Job rejected successfully");
        }
        else if (action === "accept") {
            // Accept the job: set assignment to offered and clear offered
            await pool.query("UPDATE Volunteer SET assignment = $1, offered = NULL WHERE id = $2", [offered, id]);
